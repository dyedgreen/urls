...
impl<C, A, B> diesel::query_dsl::UpdateAndFetchResults<A, B> for DieselConnection<C>
where
    C: diesel::Connection<TransactionManager = AnsiTransactionManager>
        + diesel::query_dsl::UpdateAndFetchResults<A, B>,
    C::Backend: UsesAnsiSavepointSyntax,
{
    fn update_and_fetch(&self, changeset: A) -> QueryResult<B> {
